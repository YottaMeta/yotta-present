...
- 定位：元图 —— AI 自主选择的**通用结果呈现层**（不是图表工具）；图表只是呈现形态之一。
...
- MCP server `yotta-present`（呈现，`present_result` / `present_forms`）+ `yotta-chart`（纯图表，12 个 `generate_*_chart` 工具）；SKILL.md 含「MCP：AI 自动接入」——AI 首次使用自动把两个 server 写入 mcpServers + 永久记忆护栏，按需调用、未加载降级 CLI。
- 测试：112/112（Python 3.8 + 3.13 双版本全绿；含 CLI / MCP / stdio 端到端）。
