...
<h1 align="center">yotta-chart · 元图 (YuanTu)</h1>
...
<p align="center">触发场景：需要把结果呈现得统一、可复制、美观时；用户要求卡片 / 表格 / 报告 / 美化输出时 —
...
AI 输出的内容五花八门、有的难读难复用：纯文本堆砌、乱用表格、直接丢一坨 JSON。元图 = 一个
「**呈现判断 + 美化修饰**」层：先决定这段内容用什么**形态**（卡片 / 表格 / 正文 / 图表 / 报告…），
再套上元阁统一设计语言，输出**可复制**的 Markdown / 纯文本。用户拿到的是「看着舒服、能直接复制」的结果。

它不是图表工具。图表是红海；元图的差异化 = **呈现判断 + copyable + 本地 + 通用 + CLI/MCP**。
...
- **本地 SVG**——数值分布 / 趋势 / 占比时，复用 12 图内核在本机生成 SVG，Markdown 内嵌 data URI（自包含可复制）。
- **判断可解释**——`--explain` 返回「为什么用表格 / 卡片」，避免「AI 乱选」。
- **零依赖离线**——Python 3.8+ 标准库；数据不出本机，不调外部渲染服务。
...
```bash
npx -y @yottameta/yotta-chart --agent <智能体名>     # 按智能体默认用户级目录安装（推荐）
npx -y @yottameta/yotta-chart --dir <路径>          # 装到指定目录（改过目录的智能体）
npx -y @yottameta/yotta-chart --list                # 查看 智能体 -> 默认目录
```
...
- **开源仓库中性**：MIT 开源、能力开放；商业信息走独立渠道，不写入本仓库。
