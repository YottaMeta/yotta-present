...
def _svg_wrap(body, width, height, title=None):
    ...
    if title:
        parts.append(
            ...
            'fill="#212529">%s</text>' % (width / 2, _e(title)))
    parts.append(body)
    parts.append("</svg>")
    return "\n".join(parts)
...
def _legend(entries, x, y, font_size=12, box=14, gap=6):
    ...
    for label, color in entries:
        ...
        parts.append('<text x="%d" y="%d" font-size="%d" fill="#495057">%s</text>'
...
class Cart:
    ...
    def grid(self):
        ...
        for t in self.y_ticks:
            yy = self.y(t)
            parts.append('<line x1="%d" y1="%.1f" x2="%d" y2="%.1f" stroke="#EDF2FF" stroke-width="1"/>'
                         % (self.x0, yy, self.x0 + self.plot_w, yy))
            parts.append('<text x="%d" y="%.1f" font-size="11" fill="#868E96" text-anchor="end">%s</text>'
                         % (self.x0 - 8, yy + 4, _e(_fmt_num(t))))
        return "\n".join(parts)

    def axes(self):
        return ('<line x1="%d" y1="%.1f" x2="%d" y2="%.1f" stroke="#ADB5BD" stroke-width="1"/>'
...
def _bar(labels, data, width, height, title, palette, stacked=False):
        ...
        for si, s in enumerate(series):
            ...
            for i in range(n):
                ...
                if v != 0:
                    parts.append('<rect x="%.1f" y="%.1f" width="%.1f" height="%.1f" fill="%s"/>'
                                 % (x0, y_top, bar_w, max(1.0, y_bot - y_top), _e(color)))
                parts.append('<text x="%.1f" y="%.1f" font-size="10" fill="#495057" text-anchor="middle">%s</text>'
    ...
    for i in range(0, n, step):
        ...
        parts.append('<text x="%.1f" y="%d" font-size="11" fill="#495057" text-anchor="middle">%s</text>'
    ...
    for si, s in enumerate(series):
        ...
        if len(pts) >= 2:
            ...
            parts.append('<polyline points="%s" fill="none" stroke="%s" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"/>'
                         % (poly, _e(color)))
        for x, y in pts:
            parts.append('<circle cx="%.1f" cy="%.1f" r="3.5" fill="#ffffff" stroke="%s" stroke-width="2"/>'
                         % (x, y, _e(color)))
    step = 1
    while n > 12 and n % step != 0:
        step += 1
...
def _pie(labels, data, width, height, title, palette, show_pct=True):
    ...
    parts = []
    if total <= 0:
        parts.append('<text x="%.1f" y="%.1f" font-size="14" fill="#868E96" text-anchor="middle">无数据</text>'
                     % (cx, cy))
        return _svg_wrap("\n".join(parts), width, height, title)
    angle = -90.0
    for i, v in enumerate(vals):
        ...
        parts.append('<path d="%s" fill="%s" stroke="#ffffff" stroke-width="1.5"/>' % (d, _e(color)))
        if show_pct and sweep >= 8:
            mid = math.radians((a1 + a2) / 2)
            lx = cx + r * 0.62 * math.cos(mid)
            ly = cy + r * 0.62 * math.sin(mid)
            pct = "%.0f%%" % (100.0 * v / total)
            parts.append('<text x="%.1f" y="%.1f" font-size="11" fill="#ffffff" text-anchor="middle">%s</text>'
                         % (lx, ly + 4, _e(pct)))
        angle = a2
    # 图例（右侧）
    lx = width * 0.78
    ...
    for i in range(min(8, len(labels))):
        ...
        parts.append('<text x="%.1f" y="%.1f" font-size="11" fill="#495057">%s  %s</text>'
...
def _radar(labels, data, width, height, title, palette, fill=True):
    ...
    if k < 3:
        return _svg_wrap('<text x="%d" y="%d" font-size="14" fill="#868E96" text-anchor="middle">雷达图至少需要 3 个维度</text>'
    ...
    for ring in (0.25, 0.5, 0.75, 1.0):
        ...
        parts.append('<polygon points="%s" fill="none" stroke="#EDF2FF" stroke-width="1"/>' % " ".join(pts))
    for i in range(k):
        a = math.radians(-90 + 360.0 * i / k)
        parts.append('<line x1="%.1f" y1="%.1f" x2="%.1f" y2="%.1f" stroke="#EDF2FF" stroke-width="1"/>'
    ...
    for i in range(k):
        ...
        parts.append('<text x="%.1f" y="%.1f" font-size="11" fill="#495057" text-anchor="%s">%s</text>'
...
def _scatter(data, width, height, title, palette, labels=None):
    ...
    if not all_x:
        return _svg_wrap('<text x="%d" y="%d" font-size="14" fill="#868E96" text-anchor="middle">无数据</text>'
                         % (width / 2, height / 2), width, height, title)
    ...
    for t in x_ticks:
        ...
        parts.append('<text x="%.1f" y="%d" font-size="11" fill="#868E96" text-anchor="middle">%s</text>'
...
def _histogram(data, width, height, title, palette, bins=None):
    ...
    for i, c in enumerate(counts):
        ...
        if i % max(1, bins // 8) == 0 or i == bins - 1:
            parts.append('<text x="%.1f" y="%d" font-size="10" fill="#868E96" text-anchor="middle">%s</text>'
...
def _funnel(labels, data, width, height, title, palette):
    ...
    for i in range(n):
        ...
        xr1 = cx + w_bot / 2
        color = palette[i % len(palette)]
        parts.append('<polygon points="%.1f,%.1f %.1f,%.1f %.1f,%.1f %.1f,%.1f" fill="%s" fill-opacity="%s" stroke="#ffffff" stroke-width="1.5"/>'
                     % (xl0, y0, xr0, y0, xr1, y1, xl1, y1, _e(color),
                        "%.2f" % (0.9 - 0.12 * (i % 5))))
        pct = "%.0f%%" % (100.0 * vals[i] / maxv)
        parts.append('<text x="%.1f" y="%.1f" font-size="12" fill="#ffffff" text-anchor="middle">%s  %s</text>'
...
def _waterfall(labels, data, width, height, title, palette):
    ...
    for i in range(n):
        ...
        if 0 < i < n - 1:
            ...
            parts.append('<line x1="%.1f" y1="%.1f" x2="%.1f" y2="%.1f" stroke="#ADB5BD" stroke-width="1" stroke-dasharray="3,3"/>'
...
def _sankey(data, width, height, title, palette):
    ...
    def _node_rect(node, side):
        ...
        return ('<rect x="%.1f" y="%.1f" width="10" height="%.1f" rx="2" fill="%s"/>'
                '<text x="%.1f" y="%.1f" font-size="11" fill="#495057" text-anchor="%s">%s</text>'
...
def _spreadsheet(data, width, height, title, palette, headers=None):
    ...
    for c in range(ncol):
        ...
        parts.append('<rect x="%.1f" y="%d" width="%.1f" height="%d" fill="#F1F3F5" stroke="#DEE2E6"/>'
                     % (x, top, col_w + 0.5, hdr_h))
        parts.append('<text x="%.1f" y="%d" font-size="12" font-weight="600" fill="#495057">%s</text>'
                     % (x + 8, top + hdr_h - 9, _e(txt)))
    # 数据行（斑马纹）
    for ri, r in enumerate(visible):
        y = top + hdr_h + ri * row_h
        if ri % 2 == 1:
            parts.append('<rect x="%d" y="%d" width="%d" height="%d" fill="#F8F9FA"/>'
                         % (left, y, int(usable) + 1, row_h))
        for c in range(ncol):
            x = left + col_w * c
            txt = _text_wrapped(r[c], 14) if c < len(r) else ""
            parts.append('<text x="%.1f" y="%.1f" font-size="11" fill="#212529">%s</text>'
                         % (x + 8, y + row_h - 8, _e(txt)))
    if len(rows) > max_rows:
        parts.append('<text x="%d" y="%d" font-size="11" fill="#868E96" text-anchor="middle">… 还有 %d 行未显示</text>'
...
def _treemap(data, width, height, title, palette, max_boxes=60):
    ...
    def _emit(rect, item):
        ...
        _idx[0] += 1
        parts.append('<rect x="%.1f" y="%.1f" width="%.1f" height="%.1f" fill="%s" stroke="#ffffff" stroke-width="1"/>'
                     % (x0 + 1, y0 + 1, max(1, x1 - x0 - 2), max(1, y1 - y0 - 2), _e(color)))
        fs = 11 if (x1 - x0) > 70 and (y1 - y0) > 30 else 9
        pct = "%.0f%%" % (100.0 * value / total)
        parts.append('<text x="%.1f" y="%.1f" font-size="%d" font-weight="600" fill="#ffffff">%s</text>'
                     % (x0 + 6, y0 + 16, fs, _e(_text_wrapped(label, 18))))
        if (y1 - y0) > 40:
            parts.append('<text x="%.1f" y="%.1f" font-size="10" fill="#ffffff" fill-opacity="0.85">%s</text>'
                         % (x0 + 6, y0 + 30, _e(pct)))
...
def render(chart, params=None):
    ...
    入参 params（MCP / CLI 归一化后）：
      title / labels / data / width / height / palette / filename / out / stacked /
    ...
    palette = _palette(params.get("palette"))
    ...
    result = {
        ...
        "meta": {"version": VERSION, "palette": params.get("palette") or "default",
